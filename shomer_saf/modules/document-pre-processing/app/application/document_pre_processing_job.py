...
from app.application.schemas.schema import DocumentPreProcessingRequest, DocumentPreProcessingResponse
from app.application.swagger.swagger_docs import document_preprocessing_docs
...
# Define a blueprint
document_pre_processing_bp = Blueprint("document_pre_processing", __name__)


@document_pre_processing_bp.route("/Document/PreProcessing", methods=["POST"])
@limiter.limit(rate_limit_value)
@swag_from(document_preprocessing_docs)
def document_preprocessing():
    
    try:
        payload = request.get_json(silent=True)
        request_message = DocumentPreProcessingRequest.model_validate(payload)

        request_message_dict = request_message.model_dump()

        # ---- call service ----
        logic = current_app.config["DOC_PREPROC_LOGIC"]
        ...
        # success
        response_body = DocumentPreProcessingResponse(
