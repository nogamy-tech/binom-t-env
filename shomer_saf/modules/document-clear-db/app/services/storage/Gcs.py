...
class Gcs:
    ...
    def get_blobs(self, internal_id: str) -> list[str]:
        """
        ...
        Returns:
            list[str]: The names of the blobs that match the internal_id.
        ...
        if not matching_blobs:
            print(f"No blob found in bucket '{self.bucket_name}' with internal ID '{internal_id}'.")
    ...
    def delete_folder(self, folder_name: str, time_cutoff: int) -> None:
        """
        Delete all objects under a specific 'folder' (prefix) in a GCS bucket
        that are older than a specified time cutoff.

        Args:
            folder_name (str): The prefix of the subfolder (e.g., "folder1/subfolder1/").
            time_cutoff (int): The age in hours for blobs to be deleted.
        """
        ...
        # Time threshold
        cutoff_time = datetime.now(timezone.utc) - timedelta(hours=time_cutoff)
        # List all blobs under that prefix
        blobs = self.bucket.list_blobs(prefix=folder_name)

        # Delete them
        for blob in blobs:
            if blob.name != folder_name and blob.time_created < cutoff_time:
                blob.delete()
                print(f"Deleted: {blob.name}")

        print(f"Deletion process completed for objects older than {time_cutoff} hours in '{folder_name}'.")
