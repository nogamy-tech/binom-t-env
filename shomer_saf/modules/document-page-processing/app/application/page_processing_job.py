...
from app.application.schemas.schema import PageProcessingRequest, PageProcessingResponse
...
# =============================================================================
# Define a blueprint        
page_processing_query_bp = Blueprint("page_processing_query", __name__)


@page_processing_query_bp.route("/Document/PageProcessing", methods=["POST"])
@limiter.limit(rate_limit_value)
@swag_from(document_validation_docs)  # Using shared/swagger.py
def document_query():
    try:
        payload = request.get_json(silent=True)
        request_message = PageProcessingRequest.model_validate(payload)

        request_message_dict = request_message.model_dump()

        # ---- call service ----
        logic = current_app.config["DOC_PAGEPROC_LOGIC"]
        code, message,path = logic.run_process(request_message_dict)
        ...
        # success
        response_body = PageProcessingResponse(
