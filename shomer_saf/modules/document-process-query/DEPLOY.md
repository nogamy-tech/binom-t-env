...
```bash
gcloud functions deploy cf-nogamy-shomersaf-document-process-query --gen2   --project=dgt-gcp-cgov-d-datapro   --region=me-west1   --runtime=python312   --source=.   --entry-point=flask_app   --trigger-http   --ingress-settings=internal-only   --no-allow-unauthenticated   --vpc-connector=projects/dgt-gcp-cgov-net-office-0/locations/me-west1/connectors/binom-sha-d-serverless   --egress-settings=all --service-account nogamy-sa@dgt-gcp-cgov-d-datapro.iam.gserviceaccount.com
```

```bash
gcloud functions add-iam-policy-binding cf-nogamy-shomersaf-document-process-query --project=dgt-gcp-cgov-d-datapro   --region=me-west1   --gen2   --member="allUsers"   --role="roles/cloudfunctions.invoker"
```
