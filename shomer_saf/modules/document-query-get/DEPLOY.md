...
```bash
curl -i -X POST "http://binom.dev.cgov.dgt.gcp.internal/shomer-saf/QueryGet/PollThresholdCheckResult" \
  -H "x-client-id: your-client-id" \
  -H "x-scope: your-scope" \
  -H "Authorization: Bearer $(gcloud auth print-access-token)" \
  -H "Content-Type: application/json" \
  -d '{
  "ClientRequestId": "payslip"
}'
```
curl -i -X POST "http://binom.dev.cgov.dgt.gcp.internal/shomer-saf/QueryGet/PollThresholdCheckResult" \
  -H "x-client-id: 1" \
