...
class AccountsDBTimeoutError(APIError):
    """424/500 - AccountsDB timeout (Note: Requirements show 424 but logic shows 500)"""
    def __init__(self, message="ACCOUNTS_DB_TIMEOUT", details=None):
        super().__init__(message, 500, "ACCOUNTS_DB_TIMEOUT", details)


class UnknownClientError(APIError):
    """460/500 - Unknown client (Note: Requirements show 460 but logic shows 500)"""
    def __init__(self, message="UNKNOWN_CLIENT", details=None):
        super().__init__(message, 500, "UNKNOWN_CLIENT", details)


class DBTimeoutError(APIError):
    """425/500 - DocumentsQueryJobs timeout (Note: Requirements show 425 but logic shows 500)"""
    def __init__(self, message="DB_TIMEOUT", details=None):
        super().__init__(message, 500, "DB_TIMEOUT", details)


class NoContentError(APIError):
    """204/500 - DnaTransactionId not found (Note: Requirements show 204 but logic shows 500)"""
    def __init__(self, message="NO_CONTENT", details=None):
        super().__init__(message, 500, "NO_CONTENT", details)


class InternalError(APIError):
    """500 - Internal error"""
    def __init__(self, message="INTERNAL_ERROR", details=None):
