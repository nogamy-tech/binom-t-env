...
from app.application.schemas.schema import (
    PollThresholdCheckResultRequest,
    PollThresholdCheckResultResponse,
)
...
# Define a blueprint
document_query_get_bp = Blueprint("document_query_get", __name__)


@document_query_get_bp.route("/health-check", methods=["GET"])
@swag_from(health_check_docs)
def health_check():
    return make_response(jsonify({"status": "success"}), 200)


@document_query_get_bp.route("/poll-threshold-check-results", methods=["POST"])
@limiter.limit(rate_limit_value)
@swag_from(poll_threshold_check_result_docs)
def poll_threshold_check_result():
    try:
        # Get headers
        ...
        request_message = PollThresholdCheckResultRequest.model_validate(payload)
        ...
        # ---- call service ----
        logic = current_app.config["DOC_QUERY_GET_LOGIC"]
        (
            code,
            message,
        ...
        ) = logic.run_process(request_message_dict)
        ...
        # success
        response_body = PollThresholdCheckResultResponse(
            RequestErrorCode=code,
            RequestErrorMessage=message,
            QueriesPageResults=queries_page_results,
            QueriesResults=queries_results,
            QueriesAggregatedResults=queries_aggregated_results,
